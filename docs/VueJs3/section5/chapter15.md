...
✅ parse 函数利用**有限状态自动机**完成对模板的标记化（tokenized），最终我们将得到一系列 Token。
...
✅ 通过有限自动机，我们能够将模板解析为一个个 Token，进而可以用它们构建一课 AST。在具体构建 AST 之前，我们需要思考能否简化当前的 `tokenize`函数代码。实际上，我们可以通过正则表达式来精简tokenize 函数的代码。上文之所以没有从最开始就采用正则表达式来实现，是因为**正则表达式的本质就是有限自动机**。当你编写正则表达式的时候在编写有限自动机。
